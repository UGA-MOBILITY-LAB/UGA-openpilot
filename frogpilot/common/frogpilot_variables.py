...
class FrogPilotVariables:
  ...
  def update(self, holiday_theme="stock", started=False):
    ...
    toggle.has_nnff_log = nnff_supported(toggle.car_model)
    ...
    if msg_bytes:
      ...
      toggle.has_auto_tune = LTP.useParams
      toggle.liveValid = LTP.liveValid
    else:
      toggle.has_auto_tune = False
    ...
    toggle.use_custom_friction = bool(round(toggle.friction, 2) != round(toggle.default_friction, 2)) and toggle.is_torque_car and not toggle.force_auto_tune or toggle.force_auto_tune_off
    ...
    toggle.use_custom_latAccelFactor = bool(round(toggle.latAccelFactor, 2) != round(toggle.lat_accel_factor, 2)) and toggle.is_torque_car and not toggle.force_auto_tune or toggle.force_auto_tune_off
    ...
    toggle.use_custom_steerRatio = bool(round(toggle.steerRatio, 2) != round(toggle.steer_ratio, 2)) and not toggle.force_auto_tune or toggle.force_auto_tune_off
    ...
    toggle.adjacent_lead_tracking = toggle.has_radar and (self.get_value("AdjacentLeadsUI", condition=developer_widgets) or toggle.debug_mode)
    toggle.radar_tracks = toggle.has_radar and (self.get_value("RadarTracksUI", condition=developer_widgets) or toggle.debug_mode)
    ...
    toggle.nnff = self.get_value("NNFF", condition=lateral_tuning and toggle.has_nnff_log and not toggle.is_angle_car)
    toggle.nnff_lite = self.get_value("NNFFLite", condition=not toggle.nnff and lateral_tuning and not toggle.is_angle_car)
    ...
    toggle.lane_line_width = self.get_value("LaneLinesWidth", cast=float, condition=toggle.model_ui and not toggle.debug_mode)
    toggle.path_edge_width = self.get_value("PathEdgeWidth", cast=float, condition=toggle.model_ui and not toggle.debug_mode)
    toggle.path_width = self.get_value("PathWidth", cast=float, condition=toggle.model_ui and not toggle.debug_mode)
    toggle.road_edge_width = self.get_value("RoadEdgesWidth", cast=float, condition=toggle.model_ui and not toggle.debug_mode)
